...
/*
 * Description: Test flagging of IORING_CQE_F_POLLED on a socket
 */
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
...
static int do_send(struct recv_data *rd)
{
	...
	do {
		...
		if (cqe->flags & IORING_CQE_F_POLLED) {
			fprintf(stderr, "POLLED seen prematurely!\n");
			goto err;
		}
		if (cqe->res < 0) {
			/* socket now full */
	 ...
	 * IORING_CQE_F_POLLED set, if the kernel supports this kind
	 * of notification.
	 */
	ret = io_uring_wait_cqe(&ring, &cqe);
	if (ret < 0) {
	...
	if (cqe->flags & IORING_CQE_F_POLLED)
		fprintf(stdout, "POLLED seen, kernel supports it (f=%x, p=%d)\n", rd->ring_flags, rd->pollfirst);
	else
		fprintf(stdout, "POLLED not set (f=%x, p=%d)\n", rd->ring_flags, rd->pollfirst);
	io_uring_cqe_seen(&ring, cqe);
